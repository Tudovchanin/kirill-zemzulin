...
export function starsCanvas(containerCanvas:HTMLElement, canvas:HTMLCanvasElement) {
    ...
    if(!ctx) return;
		...
		stars.forEach(star => {
			const distance = Math.sqrt((mouse.x - star.x) ** 2 + (mouse.y - star.y) ** 2); // Вычисляем расстояние
